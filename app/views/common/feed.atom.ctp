...
<?php
  echo $xml->elem('title',array(), $candy->truncate_single_line($atomTitle, 100));
  ...
  echo $xml->elem('updated', array(), (!empty($items[0])) ? $time->toRSS($EventModel->event_datetime($items[0])) : date('r'));
  ...
  foreach($items as $item) {
    echo $xml->elem('entry', array(), null, false).'>';
      echo $xml->elem('title',array(), $candy->truncate_single_line($EventModel->event_title($item), 100));
      ...
      echo $xml->elem('updated', array(), $time->toRSS($EventModel->event_datetime($item)));
      $author = $EventModel->event_author($item);
      if(!empty($author)) {
        echo $xml->elem('author', array(), null, false).'>';
          echo $xml->elem('name', array(), $candy->format_username($author));
          echo $xml->elem('email', array(), $author['mail']);
        echo $xml->closeElem();
      }
      echo $xml->elem('content', array("type" => "html"), h($candy->textilizable($EventModel->event_description($item))));
    echo $xml->closeElem();
  }
?>
</feed>