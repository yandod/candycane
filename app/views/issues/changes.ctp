...
<?php
  ...
  echo $xml->elem('updated', array(), (!empty($journals)) ? $time->toRSS($journals[0]['Journal']['created_on']) : date('r'));
  ...
  foreach($journals as $change) {
    echo $xml->elem('entry', array(), null, false).'>';
      ...
      echo $xml->elem('updated', array(), $time->toRSS($change['Journal']['created_on']));
      echo $xml->elem('author', array(), null, false).'>';
        echo $xml->elem('name', array(), $candy->format_username($change['User']));
        echo $xml->elem('email', array(), $change['User']['mail']);
      echo $xml->closeElem();
      $content = '<ul>';
      foreach($change['JournalDetail'] as $detail) {
        $content .= '<li>'.$issues->show_detail($detail, false).'</li>';
      }
      $content .= '</ul>';
      if(!empty($change['Journal']['notes'])) $content .= $candy->textilizable($change['Journal']['notes']);
