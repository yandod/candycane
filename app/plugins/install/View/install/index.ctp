<div class="install index">
    <?php
        ...
        if (is_writable(APP.'plugins')) {
            echo '<p class="success">' . __('Your plugins directory is writable.') . '('.APP.'plugins'.')</p>';
        } else {
            $check = false;
            echo '<p class="error">' . __('Your plugins directory is NOT writable.').'</p>';
        ...
        if ($check) {
            echo '<p>' . $html->link(__('Click here to begin installation'), array('action' => 'database')) . '</p>';
        } else {
