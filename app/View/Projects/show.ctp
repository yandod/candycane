 ...
 */ ?>
  <div class="box">    
    ...
    <li><?php echo $this->Html->link($tracker['name'], array('controller'=>'issues', 'action'=>'index', 'project_id'=>$this->request->data['Project']['identifier_or_id'], '?' . http_build_query(array('set_filter' => '1', 'tracker_id' => $tracker['id']), '', '&'))) ?>:
    <?php echo $tracker['open_issues_by_tracker'] ?> <?php echo $this->Candy->lwr('open', $tracker['open_issues_by_tracker']) ?>
    <?php echo __("'on'") ?> <?php echo $tracker['total_issues_by_tracker'] ?></li>
    <?php endforeach ?>
    </ul>
