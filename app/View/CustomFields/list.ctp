...
<?php foreach ($this->CustomField->sort_custom_fields_by_type($custom_fields_by_type, $tab) as $custom_field): ?>
  ...
  <td><?php echo $this->Html->link($custom_field['CustomField']['name'], array('action' => 'edit', 'id' => $custom_field['CustomField']['id'])); ?></td>
  ...
  <?php endif; ?>
    ...
    <td align="center">
      <?php 
        echo $this->Form->create(null, array('url'=>array('action'=>'destroy', 'id'=>$custom_field['CustomField']['id']), 'class'=>'button_to'));
